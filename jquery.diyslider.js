    ...
    if ( typeof (mixed) == 'object' || !mixed) {
      ...
      var data = {
        ...
        move : function(arg) {
          ...
          var to = arg[0], instant = arg[1] || false;
          ...
          if (this.options.loop && newSlide !== false && newSlide > (this.slidesCount - (this.options.display - 1)))
            newSlide = 1;
          if (this.options.loop && newSlide !== false && newSlide <= 0)
            newSlide = (this.slidesCount - (this.options.display - 1));
          ...
          if (newSlide !== false && newSlide > 0 && newSlide <= (this.slidesCount - (this.options.display - 1)) && newSlide != this.currentSlide) {
            ...
            currentSlideNode.removeClass("active").addClass("last-active");
            var newSlideNode = $(this.slides[newSlide - 1]).addClass("next-active");
            // move callback function
            var moveCallback = $.proxy(function() {
              currentSlideNode.removeClass("last-active");
              newSlideNode.removeClass("next-active").addClass("active");
            ...
            if (instant) {
              ...
              this.slidesContainer.animate(this.getSlidesContainerPosition(), {
                queue : false,
        ...
        resize : function(arg) {
          ...
          this.slidesContainer.css($.extend(this.getSlidesContainerPosition(), {
        ...
        updateOptions : function(arg) {
          var newOptions = arg[0], needCallToResize = false;
          // flag that tells us if an option that needs resize() to be run has been modified
          $.each(newOptions, $.proxy(function(k, v) {
            ...
            if (this.options.hasOwnProperty(k) && this.options[k] != v) {
              ...
              if (k == 'width' || k == 'height' || k == 'animationAxis' || k == 'display')
                needCallToResize = true;
            }
          }, this));
          if (needCallToResize)
